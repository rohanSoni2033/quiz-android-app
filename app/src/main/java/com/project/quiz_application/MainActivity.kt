...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        questionNumberRecyclerView = findViewById(R.id.quiz_question_recycler_view)
        ...
        adapter = QuestionNumberAdapter(questions.size) { currentQuestion ->
    ...
    private fun displayCurrentQuestion() {
        questionNumberRecyclerView.scrollToPosition(currentQuestionNumber - 1)
        nextQuestionButton.isEnabled =
            questions[currentQuestionNumber - 1].selectedOption > 0 && currentQuestionNumber != questions.size
        previousQuestionButton.isEnabled = currentQuestionNumber != 1

        questionNumberTextView.text = "${currentQuestionNumber} / ${questions.size}"

        val progress = (currentQuestionNumber) * 100 / questions.size
        ...
        if (questions[currentQuestionNumber - 1].selectedOption > 0) {
            val correctOption = questions[currentQuestionNumber - 1].correctOption

            if (correctOption != questions[currentQuestionNumber - 1].selectedOption) {
                when (questions[currentQuestionNumber - 1].selectedOption) {
            ...
            adapter.selectButton(currentQuestionNumber - 1, ButtonState.SELECTED)
        }
    }
    ...
    private fun optionButtonClickHandler(btn: OptionButton) {
        btn.setCheckButtonClickListener {
            val correctOption = questions[currentQuestionNumber - 1].correctOption
            questions[currentQuestionNumber - 1].selectedOption = currentSelectedOption
            nextQuestionButton.isEnabled = currentQuestionNumber != questions.size && true
            if (correctOption == currentSelectedOption) {
                btn.setCorrectOption()
                adapter.selectButton(currentQuestionNumber - 1, ButtonState.CORRECT)
            } else {
                adapter.selectButton(currentQuestionNumber - 1, ButtonState.WRONG)
                btn.setWrongOption()
                when (correctOption) {
                    1 -> optionAButton.setCorrectOption()
                    2 -> optionBButton.setCorrectOption()
